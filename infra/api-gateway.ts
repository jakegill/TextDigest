...
// kept in sync manually until we add a conversion step (TODO).
//
...
const openapiTemplate = fs.readFileSync(path.resolve("apps/api/openapi.yaml"), "utf-8");

const renderedSpec = apiServiceUrl.apply((url) =>
	Buffer.from(openapiTemplate.replaceAll("BACKEND_URL", url)).toString("base64"),
);
